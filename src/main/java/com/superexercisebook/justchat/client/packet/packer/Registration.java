...
public class Registration extends Packer {
    ...
    public Registration() {
        ...
        try {
            ...
            jsonObject.put("id", MessageTools.Base64Encode(GlobalState.config.getGeneral().server().ID()));
            jsonObject.put("name", MessageTools.Base64Encode(GlobalState.config.getGeneral().server().name()));
